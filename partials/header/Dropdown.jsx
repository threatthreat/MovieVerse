...
const Dropdown = ({ data, isLoggedIn }) => {
  return (
    <motion.div
      className="bg-[#17151e6f] backdrop-blur-[12px] border-2 border-[#4844606e] absolute top-14 right-0 rounded-2xl min-w-52 px-2 py-2 text-[14px]"
    ...
    >
      {
        isLoggedIn ? <>
          <div className="flex flex-col font-semibold ml-3 text-white">Signed in as <span>{data?.name}</span></div>
          ...
          <>
            <div className="flex items-center gap-2 hover:bg-[#262232] rounded-xl px-2 py-2 mb-2 text-slate-200 cursor-pointer" onClick={() => signinwithGoogle()}>
              <div><RxExit /></div>
              <div>Sign in</div>
            </div>
            <Link href={"/settings"} className="flex items-center gap-2 hover:bg-[#231f2f] rounded-xl px-2 py-2 text-slate-200 cursor-pointer">
